...
- [ ] タブごとにカラーテーマを設定できるようにする
  - [ ] base, primary(選択・フォーカス), secondary(補助), semantic(success/warn/error/info), grayscaleなどの状態と、タブの色調とは分けて整理する。タブの彩度は、メインのボーダーカラーとして使われるので、はっきり見えつつsemantic・primary/secondaryを邪魔しない色調にする = 低〜中彩度・中明度の「寒色寄りニュートラル」で、最低5つのテーマを用意し、タブにアサインできるようにする
