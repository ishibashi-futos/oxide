...
- [ ] `session.json` を読み込み、タブ 1 つだけでも復元できるようにする。
- [ ] `session.json` が無い場合は `cwd` を 1 タブで開く。
- [ ] タブ構成の変更後に `session.json` を保存し、次回起動で同じ構成に戻れるようにする。
- [ ] 保存先のパス決定を `OX_CONFIG_HOME` 優先で実装し、作成できなければユーザーへ通知する。
- [ ] `--in-private` と `session_restore=false` のときは復元をスキップする。
- [ ] 復元できないタブがあっても他のタブを開けるようにする。
- [ ] 終了操作で最後の保存を行い、復元結果が最新になるようにする。
