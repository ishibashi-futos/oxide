...
- [x] `AppClock` のトレイトとテスト用実装を用意する
- [x] UI 側の文字列整形の移動先を決める
- [x] 既存テストの更新対象を洗い出す

## 決定メモ

`/tab` 一覧の文字列整形は `src/ui/bottom_bar.rs` のスラッシュフィードバック表示に寄せる。

## 既存テストの更新対象

`/tab` 一覧文字列に依存するテストを更新する。

- `src/app.rs` の `slash_tests::tab_command_lists_tabs`
